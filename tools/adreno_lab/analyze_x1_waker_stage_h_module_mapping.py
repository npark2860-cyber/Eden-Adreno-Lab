#!/usr/bin/env python3
# Normalize Stage G top saved PC/LR contexts to ASLR-safe guest module+offset identities.
...
HEADER_RE = re.compile(r"\[X1-WAKERG\] frame=(?P<frame>\d+).*?producer=(?P<producer>\d+)")
TOP_RE = re.compile(
    r"top(?P<rank>[0-3])=(?P<pc>0x[0-9a-fA-F]+)/(?P<lr>0x[0-9a-fA-F]+)/"
    r"(?P<ticks>\d+)/(?P<wall>[0-9.]+)ms/(?P<slices>\d+)/(?P<share>[0-9.]+)%"
)
...
def parse_modules(lines: list[str]) -> list[ModuleRange]:
    ...
    for line in lines:
        ...
        if end - base != size:
            raise ValueError(f"module size mismatch: {line.strip()}")
    ...
    for previous, current in zip(modules, modules[1:]):
        if previous.end > current.base:
            raise ValueError(
                f"overlapping module ranges: {previous.name} and {current.name}"
            )
    return modules
...
def analyze(path: Path) -> int:
    ...
    if context_count == 0:
        raise ValueError("no [X1-WAKERG] top contexts found")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Normalize Stage G saved PC/LR contexts using Stage H module ranges"
    )

#!/usr/bin/env python3
"""Instrument exact dc95 vk_scheduler.cpp/.h without importing later behavior changes."""
...
def main() -> int:
    if len(sys.argv) != 2:
        raise SystemExit("usage: transplant_dc95_scheduler_flow_profiler.py <vk_scheduler.cpp>")
    ...
    # submit-upload barrier count is installed by the central full-flow transplant.
    text = replace_once(
        text,
        '''    chunk->MarkSubmit();
    DispatchWork();
...
void Scheduler::EndRenderPass(RenderPassEndReason reason)
    ...
    htext = replace_once(
        ...
        "reason forward declaration",
    )
    htext = replace_once(
        htext,
        '    void EndRenderPass();\n',
        '    void EndRenderPass(RenderPassEndReason reason);\n',
        "EndRenderPass declaration",
    )
    header.write_text(htext, encoding="utf-8")

    print(f"Transplanted exact dc95 scheduler flow hooks into {path}")
    return 0

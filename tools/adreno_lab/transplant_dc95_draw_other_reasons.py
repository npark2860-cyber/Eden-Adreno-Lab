...
def main() -> int:
    ...
    old = '''        buffer_cache.BindHostStageBuffers(stage);\n        PushImageDescriptors(texture_cache, guest_descriptor_queue, stage_infos[stage], rescaling,\n                             samplers_it, views_it);\n        const auto& info{stage_infos[stage]};\n'''
    new = '''        buffer_cache.BindHostStageBuffers(stage);\n        x1_other_profiler.BeginBufferCategory(\n            AdrenoProfiler::BufferCategory::OtherPushImageDescriptors);\n        PushImageDescriptors(texture_cache, guest_descriptor_queue, stage_infos[stage], rescaling,\n                             samplers_it, views_it);\n        x1_other_profiler.EndBufferCategory();\n        const auto& info{stage_infos[stage]};\n'''
    text = replace_once(text, old, new, "push image descriptors reason")

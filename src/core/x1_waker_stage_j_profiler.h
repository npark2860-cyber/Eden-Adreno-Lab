...
class X1WakerStageJProfiler final {
public:
    enum class ParentStatus : u32 {
        Valid = 0,
        ZeroFp = 1,
        UnalignedOrOverflow = 2,
        InvalidRange = 3,
        ZeroParent = 4,
        Count = 5,
    };

    static X1WakerStageJProfiler& Get() {
        static X1WakerStageJProfiler profiler;
        return profiler;
    }
    ...
    void RecordCpuSlice(u32 producer_index, u64 thread_id, u64 pc, u64 lr, u64 parent_lr,
                        ParentStatus parent_status, s64 cpu_ticks) noexcept {
        ...
        const u32 status = static_cast<u32>(parent_status);
        if (status >= ParentStatusCount) {
            producer.bad_status.fetch_add(1, std::memory_order_relaxed);
            return;
        }
        producer.parent_status_slices[status].fetch_add(1, std::memory_order_relaxed);
        producer.parent_status_ticks[status].fetch_add(ticks, std::memory_order_relaxed);

        if (parent_status != ParentStatus::Valid || parent_lr == 0 || pc == 0) {
            return;
        }

        const u32 slot = FindOrClaimContext(producer, pc, lr, parent_lr);
        if (slot == InvalidSlot) {
    ...
    void FrameEnd() noexcept {
        ...
        for (size_t producer_index = 0; producer_index < ProducerCount; ++producer_index) {
            ...
            LOG_INFO(
                HW_GPU,
                "[X1-WAKERJ] frame={} frames={} producer={} tid={:#x} slices={} cpuTicks={} "
                "validN={} validTicks={} fpZeroN={} fpZeroTicks={} fpBadN={} fpBadTicks={} "
                "rangeBadN={} rangeBadTicks={} parentZeroN={} parentZeroTicks={} "
                "overflowN={} overflowTicks={} identitySwitch={} malTicks={} badStatus={} "
                "top0={:#x}/{:#x}/{:#x}/{}/{}/{:.2f}% "
                "top1={:#x}/{:#x}/{:#x}/{}/{}/{:.2f}% "
                "top2={:#x}/{:#x}/{:#x}/{}/{}/{:.2f}% "
                "top3={:#x}/{:#x}/{:#x}/{}/{}/{:.2f}%",
                ...
                snapshot.status_slices[4], snapshot.status_ticks[4], snapshot.overflow_slices,
                snapshot.overflow_ticks, snapshot.identity_switches, snapshot.malformed_ticks,
                snapshot.bad_status, top0.pc, top0.lr, top0.parent_lr, top0.ticks, top0.slices,
                Share(top0.ticks, snapshot.total_ticks), top1.pc, top1.lr, top1.parent_lr,
                top1.ticks, top1.slices, Share(top1.ticks, snapshot.total_ticks), top2.pc, top2.lr,
                top2.parent_lr, top2.ticks, top2.slices, Share(top2.ticks, snapshot.total_ticks),
                top3.pc, top3.lr, top3.parent_lr, top3.ticks, top3.slices,
...
private:
    ...
    static constexpr size_t ParentStatusCount = static_cast<size_t>(ParentStatus::Count);
    ...
    struct ProducerState {
        ...
        std::array<std::atomic<u64>, ParentStatusCount> parent_status_slices{};
        std::array<std::atomic<u64>, ParentStatusCount> parent_status_ticks{};
    ...
    struct ProducerSnapshot {
        ...
        u64 slice_count{};
        u64 total_ticks{};
        std::array<u64, ParentStatusCount> status_slices{};
        std::array<u64, ParentStatusCount> status_ticks{};
        u64 overflow_slices{};
        u64 overflow_ticks{};
    ...
    static u32 FindOrClaimContext(ProducerState& producer, u64 pc, u64 lr,
                                  u64 parent_lr) noexcept {
    ...
    static ProducerSnapshot SnapshotAndResetWindow(ProducerState& producer) noexcept {
        ...
        for (size_t i = 0; i < ParentStatusCount; ++i) {
            snapshot.status_slices[i] =
                producer.parent_status_slices[i].exchange(0, std::memory_order_relaxed);
            snapshot.status_ticks[i] =
                producer.parent_status_ticks[i].exchange(0, std::memory_order_relaxed);
        }
        ...
        snapshot.malformed_ticks =
            producer.malformed_ticks.exchange(0, std::memory_order_relaxed);
    ...
    static void ResetProducerAll(ProducerState& producer) noexcept {
        ...
        for (size_t i = 0; i < ParentStatusCount; ++i) {
            producer.parent_status_slices[i].store(0, std::memory_order_relaxed);
            producer.parent_status_ticks[i].store(0, std::memory_order_relaxed);
        }

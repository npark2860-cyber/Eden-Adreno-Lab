...
# V6 diagnostic instrumentation inserts HOST_TEB_RESTORED between the production
# RestoreHostTebStackBounds() call and CurrentNceContext::Clear(). Anchor V15 to the
# actual transformed guest-return seam rather than the frozen-production spelling.
...
if text.count(needle) != 1:
    raise SystemExit(f"expected exactly one instrumented guest-return seam, found {text.count(needle)}")
